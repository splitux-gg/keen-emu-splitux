#!/usr/bin/env bash
# Two Enshrouded instances over goldberg LAN — Proton adaptation of bench-2seat.sh.
# Each instance: own wine prefix (clone of the validated base) + fuse-overlayed
# per-instance goldberg steam_settings (distinct steamid, listen_port, broadcasts)
# on the shared 25GB game dir. gamescope-splitux @720p30 each. GSE_FORCE_LOG so we
# can confirm the LAN broadcast/lobby handshake from goldberg's own logs.
set -uo pipefail
...
# EXPERIMENTAL gbe_fork build (self-contained, friend-join + rich-presence connect,
# honors GSE_FORCE_LOG) — the one splitux ships. Shadows the regular game-dir dll.
EXP_API="$HOME/.local/share/splitux/goldberg/win/steam_api64.dll"
SCRATCH=/tmp/splitux-together-bench/ensh-2inst
W=1280; H=720; FPS=30
...
build_overlay() {  # inst sid name lport pport
  ...
  # shadow the game-dir regular dll with the experimental friend-join build
  ...
  ( exec env ENABLE_GAMESCOPE_WSI=0 SDL_VIDEO_WAYLAND_SCALE=1 SDL_JOYSTICK_DEVICE=/dev/null \
    "$GSC" -W $W -H $H -r $FPS --force-windows-fullscreen --hide-cursor-delay 3000 -- \
      env WINEPREFIX="$d/pfx" PROTONPATH="$PROTON" GAMEID=0 \
          ...
          /usr/bin/umu-run "$d/game/enshrouded.exe" ) > "$d/gs.log" 2>&1 &
  echo $!
}
...
A=$(launch instA); echo "instA GSPID=$A  log=$SCRATCH/instA/gs.log"
sleep 18
echo "==> launching instB (joiner)"
B=$(launch instB); echo "instB GSPID=$B  log=$SCRATCH/instB/gs.log"
